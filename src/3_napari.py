...
mask_stacks = {
    image_name: np.stack([nuc_masks[x]])
...
for name, image in remove_border_cells.items():
    image
    # remove nuc near border
    nuc_filtered = clear_border(image[1][0, :, :], buffer_size=10)
    
    # stack the filtered masks
    nuc_filtered_stack = np.stack((nuc_filtered.copy()))
    stack_channels(name, masks_filtered, nuc_filtered_stack)
